...
function compose(){
  if(window.MelodixEngine){
    const params={seed:$('#seed')?.value||'melodix',style:$('#style')?.value||'Rap',substyle:$('#substyle')?.value||'Auto',mood:$('#mood')?.value||'Mélancolique',bpm:Math.max(60,Math.min(200,+$('#bpm')?.value||140)),bars:+$('#bars')?.value||32,energy:(+$('#energy')?.value||70)/100,complexity:(+$('#complexity')?.value||65)/100,counter:$('#counter')?.checked!==false,arp:$('#arp')?.checked!==false,variations:$('#variations')?.checked!==false,fills:$('#fills')?.checked!==false};
    const result=window.MelodixEngine.compose(params);
...
function setCreationMode(mode){
  ...
  if($('#analysisBadge'))$('#analysisBadge').textContent=ref?(state.reference?'READY':'WAIT'):'AUTO';
}
